...
using SpeechRecognizedWebApp.Providers.Contracts;
using SpeechRecognizedWebApp.Providers.Implementations;
...
namespace SpeechRecognizedWebApp
{
    ...
    public class SpeechRecognition : HtmlGenericControl, ICallbackEventHandler
    {
        public SpeechRecognition() : base("span")
        {
            ...
            this.Mode = RecognitionMode.SpeechRecognitionMode.Desktop;
        ...
        [DefaultValue(RecognitionMode.SpeechRecognitionMode.Desktop)]
        public RecognitionMode.SpeechRecognitionMode Mode { get; set; }

        [DefaultValue("")]
        public String OnClientSpeechRecognized { get; set; }
        ...
        [DefaultValue("")]
        public String[] Grammars { get; private set; }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            var reference = this.Page.ClientScript.GetCallbackEventReference(this, "sound",
                String.Format("function(result){{ {0}(JSON.parse(result)); }}",
                    String.IsNullOrWhiteSpace(this.OnClientSpeechRecognized) ? "void" : this.OnClientSpeechRecognized),
                String.Empty, true);
            var script =
                String.Format(
            ...
            if (sm != null)
            {
                this.Page.ClientScript.RegisterStartupScript(this.GetType(), String.Concat("process", this.ClientID),
                    String.Format(
                        "Sys.WebForms.PageRequestManager.getInstance().add_pageLoaded(function() {{ {0} }});\n", script),
                    true);
            }
            else
            {
        ...
        protected SpeechRecognitionProvider GetProvider()
        {
            switch (this.Mode)
            {
                case RecognitionMode.SpeechRecognitionMode.Desktop:
                    return (new DesktopSpeechRecognitionProvider(this));

                case RecognitionMode.SpeechRecognitionMode.Server:
